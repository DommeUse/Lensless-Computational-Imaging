...
# --- reconstruction ---
b_hat = crop(H(decrop(lensed, padded_h, padded_w), ops["P"]), h, w)
...
print(f"stats | min: {out.min().item()}, max: {out.max().item()}, mean: {out.abs().mean().item()}")
...
ax[1].imshow(to_hwc(b_hat[0]))
ax[1].set_title("b_hat = crop(H(pad(lensed)))")
